...
double Discretization::convection_u(const Matrix<double> &U, const Matrix<double> &V, int i, int j) {
    double dUU = 1 / _dx * (std::pow(((U(i, j) + U(i + 1, j)) * 0.5), 2.0) - std::pow(((U(i - 1, j) + U(i, j)) * 0.5), 2.0)) +
                    _gamma / _dx * (std::abs(U(i, j) + U(i + 1, j)) * (U(i, j) - U(i + 1, j)) * 0.5 - 
                    std::abs(U(i - 1, j) + U(i, j)) * (U(i - 1, j) - U(i, j)) * 0.5);

    double dUV = 1 / _dy * (((V(i, j) + V(i + 1, j)) * (U(i, j) + U(i, j + 1)) * 0.5) - 
                    ((V(i, j - 1) + V(i + 1, j - 1)) * (U(i, j - 1) + U(i, j)) * 0.5)) + 
                    _gamma / _dy * ((std::abs(V(i, j) + V(i + 1, j)) * (U(i, j) - U(i, j + 1)) * 0.5) - 
                    (std::abs(V(i, j - 1) + V(i + 1, j - 1)) * (U(i, j - 1) - U(i, j)) * 0.5));
...
double Discretization::convection_v(const Matrix<double> &U, const Matrix<double> &V, int i, int j) {
    double dVV = 1 / _dy * (std::pow(((V(i, j) + V(i, j + 1)) * 0.5), 2.0) - std::pow(((V(i, j - 1) + V(i, j)) * 0.5), 2.0)) +
                    _gamma / _dy * ((std::abs(V(i, j) + V(i, j + 1)) * (V(i, j) - V(i, j + 1)) * 0.5) - 
                    (std::abs(V(i, j - 1) + V(i, j)) * (V(i, j - 1) - V(i, j)) * 0.5));

    double dUV = 1 / _dx * (((U(i, j) + U(i, j + 1)) * (V(i, j) + V(i + 1, j)) * 0.5) - 
                    ((U(i - 1, j) + U(i - 1, j + 1)) * (V(i - 1, j) + V(i, j)) * 0.5)) + 
                    _gamma / _dx * ((std::abs(U(i, j) + U(i, j + 1)) * (V(i, j) - V(i + 1, j)) * 0.5) - 
                    (std::abs(U(i - 1, j) + U(i - 1, j + 1)) * (V(i - 1, j) - V(i, j)) * 0.5));
